...
interface ButtonProps {
  ...
  variant?: 'primary' | 'secondary' | 'tertiary' | 'outline' | 'goldOutline' | 'greenOutline';
  size?: 'sm' | 'md' | 'lg';
  onClick?: () => void;
  type?: 'button' | 'submit';
  className?: string;
...
const Button: React.FC<ButtonProps> = ({
  ...
  const variantStyles = {
    ...
    goldOutline:
      'bg-transparent border-2 border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-white active:bg-yellow-600',
    greenOutline:
      'bg-transparent border-2 border-[#2e8b57] text-[#2e8b57] hover:bg-[#2e8b57] hover:text-white active:bg-[#276749]',
  };
