      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-center">
        {[
          ...
          <div key={index} className="flex flex-col items-center bg-white p-6 rounded-2xl shadow-md border border-[#e5e5e5]">
            ...
            <h4 className="text-xl font-semibold text-[#2e8b57] mt-2">{value.title}</h4>
            <p className="mt-2 text-gray-700 text-sm">{value.text}</p>
          </div>
        ))}
      </div>
    ),
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <tab.icon size={20} className={isActive ? 'text-white' : 'text-[#2e8b57]'} />
                  </motion.span>
                  {tab.title}
                </motion.button>
              );
